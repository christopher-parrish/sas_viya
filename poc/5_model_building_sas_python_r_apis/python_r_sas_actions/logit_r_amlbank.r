...
### create score code
### note: may need to delete reference to target, which is included by default as output variable
### note: may need to add additional output variables in Viya to match function
score_code <- codegen(dm_model, path = paste0(output_path_folder, score_code_name), rds = paste0(model_name, '.rds'))
### to view/edit score code: file.show(paste0(output_path_folder, score_code_name), title=NULL)
### to create/view/edit score code template: create_scoreSample(output_path_folder, openFile = TRUE)
